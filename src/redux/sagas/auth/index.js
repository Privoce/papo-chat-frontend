...
function* signInPostFetch(props) {
  ...
  try {
    const response = yield sendRequest({
      url: `${constants.API.URL}${constants.API.ACTIONS.AUTH_SIGNIN}`,
...
function* signUpPostFetch(props) {
  ...
  try {
    const response = yield sendRequest({
      url: `${constants.API.URL}${constants.API.ACTIONS.AUTH_SIGNUP}`,
      method: constants.API.METHODS.POST,
      body,
    });
...
function* verifyNicknameGetFetch(props) {
  ...
  try {
    const response = yield sendRequest({
      url: `${constants.API.URL}${constants.API.ACTIONS.VERIFY_NICKNAME}`,
